...
///显示字数的label///< labelNumber
@property (nonatomic, strong, readwrite) UILabel * labelNumber;
...
- (void)initialize
{
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.placeholderLabel
                                                     ...
                                                     attribute:NSLayoutAttributeWidth
                                                    multiplier:1.0
                                                      constant:-kFSTextViewPlaceholderHorizontalMargin*3/2]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.placeholderLabel
...
- (void)layoutSubviews {
    ...
    CGRect frame = self.labelNumber.frame;
    frame.origin.x = self.bounds.size.width-frame.size.width-kFSTextViewPlaceholderHorizontalMargin;
    frame.origin.y = self.bounds.size.height-frame.size.height-kFSTextViewPlaceholderVerticalMargin + self.contentOffset.y;
...
- (UILabel *)labelNumber {
    if (!_labelNumber) {
        ...
        _labelNumber.textColor = [UIColor lightGrayColor];
...
- (void)setMaxLength:(NSUInteger)maxLength
{
    ...
    self.text = self.text;
    if (_maxLength != 0 && _maxLength != NSUIntegerMax) {
        self.labelNumber.hidden = false;
        self.labelNumber.text = [[[@(self.text.length) stringValue] stringByAppendingString:@"/"] stringByAppendingString:[@(maxLength) stringValue]];
    }else{
...
- (void)textDidChange:(NSNotification *)notification
{
    ...
    BOOL isNoText = [@(self.text.length) boolValue];
    self.placeholderLabel.hidden = isNoText;
    ///不是最大值和0的时候
    BOOL isMaxNoZero = _maxLength != NSUIntegerMax  && _maxLength != 0;
    if (!isNoText && isMaxNoZero) {
        self.labelNumber.text = [@"0/" stringByAppendingString:[@(_maxLength) stringValue]];
    }
...
//    }
    ...
    if (isMaxNoZero && self.text.length > 0) {
        ...
        if (!self.markedTextRange && self.text.length > _maxLength) {
            ...
            self.labelNumber.textColor = self.text.length == _maxLength ? [UIColor redColor]:[UIColor lightGrayColor];
            self.labelNumber.text = [[[@(self.text.length) stringValue] stringByAppendingString:@"/"] stringByAppendingString:[@(_maxLength) stringValue]];
        }
    }
